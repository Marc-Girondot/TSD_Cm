...
```{r}
load(file = file.path("dataIn", "green.Rdata"))
load(file = file.path("dataIn", "green_full.Rdata"))
load(file = file.path("dataIn", "TempTot.Rdata"))
```
...
## Get the 2 m high air temperature from ECMWF
...
# Plot the field data for nests

Take care, the plot takes 15 minutes to plot.
...
# These values have be copied from the fitted model
...
Take care, this is based on ML; not Bayesian.

```{r label=compareAIC}
kAICc <- compare_AICc(Cm4p=resultNest_4p_80_CmFlorida, 
                      Cm6p=resultNest_6p_80_CmFlorida, silent = TRUE)
compare_AICc(Cm4p=resultNest_4p_80_CmFlorida, 
             Cm6p=resultNest_6p_80_CmFlorida)
```

The probability that the 4 parameters model is the best one among the two tested models is `r kAICc["Cm4p", "Akaike_weight"]`. 
...
Note that the number of iterations is a little bit short and it produced bimodal distribution.
...
```{r}
load(file=file.path("dataOut", "Gompertz", "result_mcmc_6p_80_CmFlorida.Rdata"))
...
kWAIC <- loo_model_weights(list(p4p=result_mcmc_4p_80_CmFlorida_loo, 
                                p6p=result_mcmc_6p_80_CmFlorida_loo), 
                           method = "stacking")
kWAIC
```

Using ML, the probability that the 4 parameters model is the best one among the two tested models is `r kAICc["Cm4p", "Akaike_weight"]`. 

Using Bayesian analysis and stacking, the probability that the 4 parameters model is the best one among the two tested models is `r specify_decimal(unname(kWAIC["p4p"]), 2)`. 
...
```{r}
load(file=file.path("dataOut", "tsd", "tsd_flexit.Rdata"))
load(file=file.path("dataOut", "tsd", "flexit_CM.Rdata"))
```
...
plot_errbar(males/sexed, sr[names(sexed), "TSP.TimeWeighted.sexratio.quantile_0.5"], 
            ...
            ylab="Predicted sex ratio", xlab="Observed sex ratio", 
            main="Average temperature during TSP")
...
plot_errbar(males/sexed, sr[names(sexed), "TSP.GrowthWeighted.sexratio.quantile_0.5"], 
            ...
            las=1, bty="n", xlim=c(0, 1), ylim=c(0, 1), 
            ylab="Predicted sex ratio", xlab="Observed sex ratio", 
            main="Growth weight average temperature during TSP")
segments(x0=0, x1=1, y0=0, y1=1, lty=2)
...
plot_errbar(males/sexed, sr[names(sexed), "TSP.TimeWeighted.GrowthRateWeighted.sexratio.quantile_0.5"], 
            ...
            las=1, bty="n", xlim=c(0, 1), ylim=c(0, 1), 
            ylab="Predicted sex ratio", xlab="Observed sex ratio", 
            main="Growth rate weight average temperature during TSP")
segments(x0=0, x1=1, y0=0, y1=1, lty=2)
...
```{r}
load(file = file.path("dataOut", "Gompertz", "TW_TSP_fixed_flexit.Rdata"))
TW_TSP_fixed_flexit$AICc
load(file = file.path("dataOut", "Gompertz", "TW.GW_TSP_fixed_flexit.Rdata"))
TW.GW_TSP_fixed_flexit$AICc
load(file = file.path("dataOut", "Gompertz", "GW_TSP_fixed_flexit.Rdata"))
GW_TSP_fixed_flexit$AICc
```


## First try with TRNS as a trigonometric model

The TRNS is sometimes difficult to fit. The trigonometric model is easier to implement. It will serve as a guide for more pertinent models.
...
TRNS_Trigo_TSP_fixed_flexit <- STRN(Initial_STRN = c('Peak' = -7.5875654157444652, 
                                                     'LengthB' = 89.22687204054813, 
                                                     'LengthE' = 36.873396687900652, 
                                                     'Min' = 0.0033493993765771066), 
                                    ...
                                    itnmax = 1000                                                                ,
                                    method = c("BFGS")                                                    ) 
save(TRNS_Trigo_TSP_fixed_flexit, file = file.path("dataOut", "Gompertz", "TRNS_Trigo_TSP_fixed_flexit.Rdata"))
```


```{r}
load(file = file.path("dataOut", "Gompertz", "TRNS_Trigo_TSP_fixed_flexit.Rdata"))
TRNS_Trigo_TSP_fixed_flexit$par
...
plot(20:40, getFromNamespace(".SSM", ns="embryogrowth")(T=20:40, parms=TRNS)[[1]]*1000, type="l", ylab ="TRNS", xlab="Incubation temperature", las=1, bty="n")

p <- ChangeSSM(parameters = TRNS, 
               initial.parameters = c('DHA' = -446.94395858618122, 
                                      'DHH' = 685.20357374674995, 
                                      'T12H' = 552.20446499322202), 
               fixed.parameters = c('Rho25' = 100))

plot(20:40, getFromNamespace(".SSM", ns="embryogrowth")(T=20:40, parms=c(p$par, 'Rho25' = 100))[[1]]*1000, type="l", ylab ="TRNS", xlab="Incubation temperature", las=1, bty="n")
```

## TRNS

The trigonometric model is replaced by Schoofield model.
...
```{r}
load(file = file.path("dataOut", "Gompertz", "TRNS_TSP_fixed_flexit.Rdata"))
...
```{r}
load(file=file.path("dataOut", "StatsCm_TRNS.Rdata"))
...
```{r}
load(file = file.path("dataOut", "Gompertz", "TRNS_Force_TSP_fitted_flexit.Rdata"))
...
```{r}
load(file=file.path("dataOut", "StatsCm_TRNS_Force_TSP.Rdata"))
...
```{r}
load(file = file.path("dataOut", "Gompertz", "TRNS_TSP_fitted_flexit.Rdata"))
...
```{r}
load(file=file.path("dataOut", "StatsCm_TRNS_TSP.Rdata"))
...
```{r}
load(file = file.path("dataOut", "Gompertz", "Force_TSP_fitted_flexit.Rdata"))
...
```{r}
load(file=file.path("dataOut", "StatsCm_Force_TSP.Rdata"))
sr <- StatsCm_TRNS_Force_TSP$summary[, c("Series", "TSP.GrowthWeighted.STRNWeighted.sexratio.quantile_0.025",
                                         "TSP.GrowthWeighted.STRNWeighted.sexratio.quantile_0.5", 
                                         "TSP.GrowthWeighted.STRNWeighted.sexratio.quantile_0.975")]
...
```{r}
load(file = file.path("dataOut", "Gompertz", "TSP_fitted_flexit.Rdata"))
...
```{r}
load(file = file.path("dataOut", "Gompertz", "Force_TSP_fixed_flexit.Rdata"))
...
```{r}
load(file=file.path("dataOut", "StatsCm_Force.Rdata"))
...
## Comparison of TRNS, Force and fitted TSP models
...
```{r}
load(file = file.path("dataOut", "Gompertz", "TW_TRNS_Force_TSP_fixed_flexit.Rdata"))
